...
## MỤC LỤC CHƯƠNG TRÌNH (TABLE OF CONTENTS)
*   **[1. Lời mở đầu & Bối cảnh Nghiên cứu](#1-lời-mở-đầu--bối-cảnh-nghiên-cứu)**
*   **[2. Kiến trúc Đa tác tử (Multi-Agent with LangGraph)](#2-kiến-trúc-đa-tác-tử-multi-agent-workflow-with-langgraph)**
    *   *2.1 Định nghĩa Trạng thái Chung (Shared State Structure)*
    *   *2.2 Tác tử Tìm kiếm Hỗn hợp (ArxivSearchAgent)*
    *   *2.3 Tác tử Phê bình Chuyên sâu (PaperCriticAgent)*
    *   *2.4 Tác tử Tổng hợp Lộ trình (LiteratureReviewAgent)*
*   **[3. Hệ thống Trích xuất Thông tin Lai & Vector Embedding (Hybrid RAG System)](#3-hệ-thống-trích-xuất-thông-tin-lai--vector-embedding-hybrid-rag-system)**
    *   *3.1 Cơ chế Phân đoạn Văn bản phân cấp (Parent-Child Chunking)*
    *   *3.2 Quy trình Mã hóa và Tìm kiếm Ngữ nghĩa Hỗn hợp*
    *   *3.3 Thuật toán Tìm kiếm Vector Ngữ nghĩa (Semantic Search)*
    *   *3.4 Cơ chế Dự phòng Cục bộ (Lexical Fallback)*
*   **[4. Bản đồ Học thuật 2D & Thuật toán Học chủ động (Scholar Maps & Active Learning)](#4-bản-đồ-học-thuật-2d--thuật-toán-học-chủ-động-scholar-maps--active-learning)**
    *   *4.1 Thuật toán Trực quan hóa Bản đồ Học thuật (t-SNE Reduction)*
    *   *4.2 Thuật toán Học chủ động (Active Learning Selector)*
*   **[5. Hệ thống Gợi ý Cá nhân hóa với định dạng dữ liệu JSON chuẩn](#5-hệ-thống-gợi-ý-cá-nhân-hóa-với-định-dạng-dữ-liệu-json-chuẩn)**
*   **[6. Thiết kế phần mềm & Đồng bộ hóa giao diện (UI/UX Synchronization)](#6-thiết-kế-phần-mềm--đồng-bộ-hóa-giao-diện-uiux-state-synchronization)**
*   **[7. Cấu hình kiểm thử thực nghiệm (Experimental Configurations)](#7-cấu-hình-kiểm-thử-thực-nghiệm-experimental-configurations)**
*   **[8. Phân tích đóng góp của Đề tài so với Bản cơ sở (Contributions vs. Baseline)](#8-phân-tích-đóng-góp-của-đề-tài-so-với-bản-cơ-sở-contributions-vs-baseline)**
*   **[9. Hướng dẫn báo cáo và thuyết trình kết quả cải tiến (Presentation & Defense Guide)](#9-hướng-dẫn-báo-cáo-và-thuyết-trình-kết-quả-cải-tiến-presentation--defense-guide)**
*   **[Kết luận và Hướng phát triển (Conclusion & Future Work)](#kết-luận-và-hướng-phát-triển-conclusion--future-work)**
...
## 9. HƯỚNG DẪN BÁO CÁO VÀ THUYẾT TRÌNH KẾT QUẢ CẢI TIẾN (PRESENTATION & DEFENSE GUIDE)

Để hỗ trợ học viên báo cáo thuận lợi trước Giảng viên hướng dẫn hoặc Hội đồng Khoa học, dưới đây là phần phân tích chi tiết cốt lõi kỹ thuật, lý do cải tiến và mẫu phát biểu trực quan cho từng đóng góp chính:

### 9.1 Nâng cấp Hệ thống RAG với Tìm kiếm Ngữ nghĩa Neural, Batching & Caching
*   **Bản chất kỹ thuật:** Thay thế thuật toán so khớp từ khóa lexical cũ (TF-IDF) sang tìm kiếm ngữ nghĩa dày đặc bằng **Dense Vector Embedding** thông qua mô hình `text-embedding-004` (768 chiều). Khoảng cách ngữ nghĩa giữa câu hỏi truy vấn $\mathbf{u}$ và Child Chunk văn bản $\mathbf{v}$ được đo lường bằng phép toán **Cosine Similarity**.
*   **Tại sao lại tốt hơn:** TF-IDF chỉ tìm kiếm từ ngữ trùng khớp nguyên tự (ví dụ: tìm từ "robot" thì chỉ khớp trang có chữ "robot"). Dense Vector Embedding mã hóa ngữ nghĩa (ví dụ: câu hỏi về "cánh tay máy Franka" vẫn tự động khớp với các đoạn văn viết về "manipulator system" hay "actuators" nhờ sự tương đồng về hướng vector).
*   **Kỹ thuật tối ưu hóa cốt lõi:**
    *   *Batching:* Gom toàn bộ các Child Chunks của trang PDF gửi đi mã hóa trong duy nhất một yêu cầu `batchEmbedContents` để tránh nghẽn kết nối mạng.
    *   *Disk Caching:* Lưu đệm file vector JSON cục bộ trên đĩa để truy xuất RAM tức thì trong các lượt chat tiếp theo.
*   **Mẫu thuyết trình đề xuất:**
    > *"Em đã nâng cấp hệ thống RAG đọc hiểu PDF của bản cơ sở từ tìm kiếm từ khóa thuần túy sang **Tìm kiếm Ngữ nghĩa Sâu (Semantic Search)** bằng cách sử dụng **Vector Embedding**. Nhờ đó hệ thống có thể hiểu được ngữ cảnh câu hỏi của người dùng ngay cả khi họ dùng các thuật ngữ đồng nghĩa. Đồng thời, em đã tối ưu hóa hiệu năng bằng kỹ thuật **Batch Embedding** để gom yêu cầu mạng và **Disk Caching** để lưu trữ vector đặc trưng dưới dạng đệm cục bộ, giúp giảm 90% độ trễ truy xuất dữ liệu."*

### 9.2 Thiết kế Đa tác tử và Lộ trình Thực nghiệm (Multi-Agent Workflow)
*   **Bản chất kỹ thuật:** Ứng dụng mô hình **Đa tác tử (Multi-Agent Architecture)** bằng LangGraph để chia nhỏ vai trò của LLM thành các Agent chuyên môn hóa giao tiếp qua biến trạng thái chung `AgentState`: Tìm kiếm thông tin $\rightarrow$ Phê bình song song đa luồng $\rightarrow$ Tổng hợp lộ trình.
*   **Tại sao lại tốt hơn:** Một prompt đơn lẻ (Single prompt) gửi cho LLM thường chỉ tạo ra phản hồi tóm tắt lý thuyết nông, thiếu chiều sâu thực nghiệm. Việc chia nhỏ các bước giúp kiểm soát tốt chất lượng từng đầu ra, hạn chế tối đa ảo giác (hallucination).
*   **Kỹ thuật tối ưu hóa cốt lõi:** Chạy đa luồng song song (`ThreadPoolExecutor`) đối với tác tử phê bình tài liệu để tối ưu hóa thời gian xử lý đồ thị.
*   **Mẫu thuyết trình đề xuất:**
    > *"Trong phần AI Workspace, em đã thiết kế lại quy trình tổng hợp tài liệu theo kiến trúc **Đa tác tử (Multi-Agent)** sử dụng **LangGraph** để chia nhỏ vai trò của LLM: Tác tử tìm kiếm có cá nhân hóa $\rightarrow$ Tác tử phê bình sâu chạy đa luồng song song để trích xuất toán học và thông số huấn luyện $\rightarrow$ Tác tử tổng hợp lộ trình kỹ thuật.*
    *Đầu ra của quy trình không chỉ là tóm tắt lý thuyết, mà là một **Lộ trình thực nghiệm khoa học khả thi (Vietnamese Research Roadmap)** chia rõ các mốc thời gian theo tuần/tháng, chỉ dẫn cấu hình môi trường CUDA/GPU cụ thể cho từng giai đoạn và phương án dự phòng giảm tài nguyên bộ nhớ VRAM."*

### 9.3 Định tuyến API Lai và Đảm bảo Tính sẵn sàng cao (Hybrid API Routing)
*   **Bản chất kỹ thuật:** Phát triển bộ router tự động kiểm tra định dạng API Key (Gemini `AIza...` hoặc Groq `gsk_...`), tự động cấu hình lại payload, kiểu dữ liệu JSON đầu ra và thêm Chrome Spoofing headers để gọi dịch vụ tương ứng.
*   **Tại sao lại tốt hơn:** Tránh lỗi Single Point of Failure (lỗi đơn điểm) khi một API Key hết quota sử dụng hoặc bị nghẽn mạng từ máy chủ ngoài.
*   **Mẫu thuyết trình đề xuất:**
    > *"Để đảm bảo **tính sẵn sàng cao (High Availability)** và tối ưu chi phí cho hệ thống, em đã thiết kế cơ chế **Hybrid API Routing** hỗ trợ chuyển đổi linh hoạt giữa Google Gemini và Groq Llama 3.3. Khi một dịch vụ gặp sự cố hoặc hết quota sử dụng, hệ thống có khả năng tự động định tuyến lại payload dữ liệu sang nhà cung cấp dịch vụ dự phòng mà không yêu cầu người dùng phải khởi động lại server."*

### 9.4 Bộ biên dịch Markdown-to-JSX trên Frontend
*   **Bản chất kỹ thuật:** Viết bộ parser nội bộ bằng React/JS thay thế thẻ `<pre>` hiển thị thô.
*   **Tại sao lại tốt hơn:** Tăng tính thẩm mỹ khoa học, tự động vẽ các bảng so sánh học thuật, các danh sách mốc thời gian dưới dạng HTML Table chuyên nghiệp.
*   **Mẫu thuyết trình đề xuất:**
    > *"Về phần hiển thị phía máy khách (Frontend UI/UX), em đã xây dựng một **Bộ biên dịch Markdown-to-JSX (Custom Markdown Parser)** nhằm loại bỏ hoàn toàn các ký tự thô của LLM và chuyển đổi trực tiếp văn bản markdown sang các thẻ tiêu đề, danh sách phân cấp và các bảng biểu dữ liệu trực quan sinh động. Điều này giúp nâng cao đáng kể trải nghiệm đọc báo cáo khoa học của người dùng."*
